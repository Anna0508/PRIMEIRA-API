...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if not usuario or usuario.get("active") == False:
        raise HTTPException(
    ...
    if not verificar_senha(form_data.password, usuario["password"]):
        logger.warning(f"Falha de login: {form_data.username}", extra={"user": "Sistema"})
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="E-mail ou senha incorretos.",
            headers=headers
    ...
    logger.info(f"Usuário logado com sucesso: {form_data.username}", extra={"user": usuario.get("name")})
...
def listar_usuarios(usuario_atual: dict = Depends(obter_usuario_atual)):
    ...
    usuarios_seguros = {
        email: {chave: valor for chave, valor in dados_user.items() if chave != "password"}
...
async def criar_usuario(dados: UsuarioCriar, usuario_atual: dict = Depends(obter_usuario_atual)):
    ...
    if usuario_atual.get("role") != "admin":
        logger.error(f"FALHA: Sem permissão", extra={"user": quem})
        raise HTTPException(status_code=403, detail="Você não possui acesso.")

    if dados.email in USUARIOS_DB:
        logger.error(f"FALHA: E-mail cadastrado", extra={"user": quem})
    ...
    logger.info(f"QUEM: {quem}, AÇÃO: CRIAR_USUARIO, EM QUEM: {dados.email}, STATUS: SUCESSO", extra={"user": quem})
    return {"mensagem": f"Usuário {dados.email} criado com sucesso."}

@app.put("/usuarios/editar")
async def editar_usuario(email_alvo: str, dados: UsuarioEditar, usuario_atual: dict = Depends(obter_usuario_atual)):
    quem = usuario_atual.get("name", "Admin")
    ...
    logger.info(
        f"Alterou a conta de [{email_alvo}]. Antigos: {dados_antigos.get('role')} Novos: {USUARIOS_DB[email_alvo].get('role')}",
        extra={"user": quem}
    )
